...
//        }
        ...
        Long tokenVersionFromJwt = ClaimUtils.getClaimAsLong(jwt, JwtClaimConstants.Token.VERSION);
        ...
        String subjectType = userId != null ? SystemConstants.SubjectType.USER : SystemConstants.SubjectType.MEMBER;


        String versionKeyFromRedis = stringRedisTemplate.opsForValue().get(versionKey);
        if (versionKeyFromRedis == null) {
        ...
        * */
        Long currentTokenVersionFromRedis;
        try {
            currentTokenVersionFromRedis = Long.valueOf(versionKeyFromRedis);
        } catch (Exception e) {
            log.error("Redis 读取 token version 失败，降级放行，subjectId={}", subjectId, e);
            // ✅ 降级：放行（或按你业务策略拒绝）
            filterChain.doFilter(request, response);
            return;
        }

        log.info("【JwtVersionFilter】令牌版本校验阶段, subjectType={}，subjectId={}, tokenVersionFromJwt={}, currentTokenVersionFromRedis={}",
                subjectType, subjectId, tokenVersionFromJwt,currentTokenVersionFromRedis);

        if (currentTokenVersionFromRedis == null || !currentTokenVersionFromRedis.equals(tokenVersionFromJwt)) {
            log.warn("Token version 失效，拒绝访问，subjectId={}, tokenVersionFromJwt={}, currentTokenVersionFromRedis={}",
                    subjectId, tokenVersionFromJwt, currentTokenVersionFromRedis);
            throw new OAuth2AuthenticationException(
                    new OAuth2Error(OAuth2ErrorCodes.INVALID_TOKEN, "登录已失效，请重新登录", null)
            );
        }
    ...
    private boolean isWhitelisted(HttpServletRequest request) {
        if (resourceSecurityProperties.getWhitelistPaths() == null) return false;
