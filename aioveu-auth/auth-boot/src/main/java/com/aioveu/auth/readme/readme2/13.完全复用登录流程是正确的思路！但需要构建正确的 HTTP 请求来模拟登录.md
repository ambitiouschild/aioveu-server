...
public class TenantSwitchServiceImpl implements TenantSwitchService {
    ...
    public Result<Authentication> switchTenant(Long tenantId) {
        try {
            ...
            // 4. 获取用户在新租户下的权限信息
            UserAuthInfoWithTenantId userAuthCredentialsForSystem = tenantFeignClient.getUserAuthInfoWithTenantId(
                details.getUsername(), tenantId);
            
            // 5. 构建新的用户详情
            SysUserDetails newDetails = buildNewUserDetails(details, userAuthCredentialsForSystem, tenantId);
    ...
    private SysUserDetails buildNewUserDetails(
            ...
            UserAuthInfoWithTenantId userAuthCredentialsForSystem,
            Long tenantId) {
        
        SysUserDetails newDetails = new SysUserDetails(userAuthCredentialsForSystem);
