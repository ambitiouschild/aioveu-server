package com.aioveu.pay.aioveu06PayFlow.utils;
...
/**
 * @ClassName: PayFlowNoGenerator
 * @Description TODO 支付流水号生成器
 * @Author 可我不敌可爱
 * @Author 雒世松
...
@Component
public class PayFlowNoGenerator {
     ...
     */
    public static String generatePayFlowNo(Long memberId) {
     ...
     */
    public static String generatePayFlowNoRandom(Long memberId) {
        // 1. 时间戳部分（14位）
     ...
     */
    public static String generateSnowflakeStylePayFlowNo(Long memberId, Integer workerId) {
        // 1. 时间戳（10位，毫秒级）
     ...
     */
    public static String generateUUIDPayFlowNo(Long memberId) {
        // 1. 日期部分（8位）
     ...
     */
    public static String generateReversiblePayFlowNo(Long memberId) {
        // 1. 时间戳（13位，毫秒级）
        long timestamp = System.currentTimeMillis();
    ...
    /**
     * 使用Redis生成分布式唯一退款单号  高并发场景：推荐方案2（Redis分布式序列）
     * 格式: 日期(8位) + Redis序列(6位) + 随机数(6位)
     */
    public String generateDistributedPayFlowNo(Long memberId) {
        // 1. 日期部分
        String datePart = LocalDateTime.now().format(DATE_FORMATTER);

        // 2. Redis分布式序列（每天重置）
        String sequenceKey = REFUND_SEQUENCE_KEY + datePart;
